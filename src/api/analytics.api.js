...
export const fetchEquityCurve = async (workspaceId, period = "weekly") => {
  ...
  const { data } = await api.get(
    `/workspaces/${workspaceId}/analytics/equity-curve/`,
    { params: { period } }
  );
  return data;
};
...
export const fetchWinLossDistribution = async (workspaceId) => {
  ...
  const { data } = await api.get(
    `/workspaces/${workspaceId}/analytics/win-loss-distribution/`
  );
  return data;
};
...
export const fetchPnlBySetup = async (workspaceId) => {
  ...
  const { data } = await api.get(`/workspaces/${workspaceId}/analytics/pnl-by-setup/`);
  return data;
};
